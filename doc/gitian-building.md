...
*Setup instructions for a Gitian build of ERPcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ERPcoin
...
- [Building ERPcoin Core](#building-erpcoin-core)
...
Clone the git repositories for ERPcoin Core and Gitian.
...
in the ERPcoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ERPcoin Core
----------------

To build ERPcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ERPcoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/erpcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/erpcoinpay/erpcoin
...
For example:
```bash
URL=https://github.com/crowning-/erpcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit erpcoin=${COMMIT} --url erpcoin=${URL} ../erpcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit erpcoin=${COMMIT} --url erpcoin=${URL} ../erpcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit erpcoin=${COMMIT} --url erpcoin=${URL} ../erpcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the erpcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../erpcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/erpcoin
SIGPATH=/some/root/path/erpcoin-detached-sigs

./bin/gbuild --url erpcoin=${BTCPATH},signature=${SIGPATH} ../erpcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/erpcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/erpcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/erpcoin-osx-build.assert
```
...
[erpcoin/gitian.sigs](https://github.com/erpcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
