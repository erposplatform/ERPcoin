...
ERPcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ ERP, _12_ times over the course of a year totaling _24000_ ERP.

**Warning: if you change any fields within this command, the collateral transaction will become invalid.**

Format: ```mngovernance prepare proposal-name url payment-count block-start erpcoin-address monthly-payment-erpcoin```
...
Format: ```mngovernance submit proposal-name url payment-count block-start erpcoin-address monthly-payment-erpcoin fee-tx```
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` ERP to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using erpcoin.conf setup)
