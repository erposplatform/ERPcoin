...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "erpcoin.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for erpcoind to start. This means that RPC is accessible and fully initialized.
    Raise an exception if erpcoind exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('erpcoind exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        # Create cache directories, run erpcoinds:
        for i in range(MAX_NODES):
            datadir=initialize_datadir(cachedir, i)
            args = [ os.getenv("ERPD", "erpcoind"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: erpcoind started, waiting for RPC to come up")
    ...
    for i in range(num_nodes):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in erpcoin.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start a erpcoind and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("ERPD", "erpcoind")
    ...
    bitcoind_processes[i] = subprocess.Popen(args, stderr=stderr)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: erpcoind started, waiting for RPC to come up")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    """
    Start multiple erpcoinds, return RPC connections to them
    """
...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s ERP too low! (Should be %s ERP)"%(str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s ERP too high! (Should be %s ERP)"%(str(fee), str(target_fee)))
