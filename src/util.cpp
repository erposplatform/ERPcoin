...
const char * const BITCOIN_CONF_FILENAME = "erpcoin.conf";
const char * const BITCOIN_PID_FILENAME = "erpcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "erpcoin" is a composite category enabling all ERPcoin-related debug output
                if(ptrCategory->count(std::string("erpcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ERPcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ERPcoin
    // Mac: ~/Library/Application Support/ERPcoin
    // Unix: ~/.erpcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ERPcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ERPcoin";
#else
    // Unix
    return pathRet / ".erpcoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty erpcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override erpcoin.conf
