...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total erpcoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized erpcoin balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "erpcoin|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "erpcoin")
            + HelpExampleRpc("debug", "erpcoin+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given erpcoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The erpcoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The erpcoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ERPcoin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are erpcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) erpcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The erpcoin address to use for the signature.\n"
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The difference of Simbian\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The number of Simbian of the output\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in Simbian\n"
            "  \"received\"  (string) The total number of Simbian received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* ERPcoin features */
    { "erpcoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "erpcoin",               "spork",                  &spork,                  true,  {"value"} },
