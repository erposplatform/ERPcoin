...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in erpcoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("erpcoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* ERPcoin features */
    { "erpcoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "erpcoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "erpcoin",               "gobject",                &gobject,                true,  {} },
    { "erpcoin",               "voteraw",                &voteraw,                true,  {} },
