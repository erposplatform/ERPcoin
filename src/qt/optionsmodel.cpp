...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ERP);
    ...
    if (!settings.contains("nPrivateSendAmount")) {
        ...
        if (!settings.contains("nAnonymizeERPcoinAmount"))
            settings.setValue("nPrivateSendAmount", DEFAULT_PRIVATESEND_AMOUNT);
        else
            settings.setValue("nPrivateSendAmount", settings.value("nAnonymizeERPcoinAmount").toInt());
    }
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in erpcoin.cpp during shotdown to also remove the window positions
