...
{
    return QCoreApplication::translate("erpcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in erpcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in erpcoin.qrc)
...
/** Class encapsulating ERPcoin Core startup and shutdown.
...
/** Main ERPcoin application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // erpcoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. ERPcoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(erpcoin);
    Q_INIT_RESOURCE(erpcoin_locale);
    ...
    /// 6. Determine availability of data directory and parse erpcoin.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("ERPcoin Core"),
    ...
    // erpcoin: links repeatedly have their payment requests routed to this process:
