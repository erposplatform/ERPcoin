...
pkgdatadir = $(datadir)/erpcoincore
pkgincludedir = $(includedir)/erpcoincore
pkglibdir = $(libdir)/erpcoincore
pkglibexecdir = $(libexecdir)/erpcoincore
...
CONFIG_HEADER = $(top_builddir)/src/config/erpcoin-config.h
CONFIG_CLEAN_FILES = liberpcoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/liberpcoinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/erpcoin-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/ERPcoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/ERPcoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/ERPcoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/ERPcoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = erpcoin-cli
BITCOIN_DAEMON_NAME = erpcoind
BITCOIN_GUI_NAME = erpcoin-qt
BITCOIN_TX_NAME = erpcoin-tx
...
COPYRIGHT_HOLDERS_FINAL = The ERPcoin Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = ERPcoin Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/ERPcoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = erpcoincore
PACKAGE_BUGREPORT = https://github.com/erpcoinpay/erpcoin/issues
PACKAGE_NAME = ERPcoin Core
PACKAGE_STRING = ERPcoin Core 1.0.0
PACKAGE_TARNAME = erpcoincore
PACKAGE_URL = https://erpcoin.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/ERPcoin
abs_srcdir = /tmp/ERPcoin
abs_top_builddir = /tmp/ERPcoin
abs_top_srcdir = /tmp/ERPcoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/ERPcoin/build-aux/install-sh
...
pkgconfig_DATA = liberpcoinconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/erpcoin-cli.bash-completion \
	       $(top_srcdir)/contrib/erpcoin-tx.bash-completion \
	       $(top_srcdir)/contrib/erpcoind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_erpcoin_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_erpcoin_coverage.info test_erpcoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/ERPcoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/erpcoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/erpcoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/erpcoin-config.h
$(top_srcdir)/src/config/erpcoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/erpcoin-config.h src/config/stamp-h1
liberpcoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/liberpcoinconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/ERPcoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ERPcoin-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ERPcoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_erpcoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_erpcoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_erpcoin_filtered.info: test_erpcoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_erpcoin_filtered.info
...
#test_erpcoin_coverage.info: baseline_filtered_combined.info test_erpcoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_erpcoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_erpcoin_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_erpcoin_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_erpcoin.coverage/.dirstamp:  test_erpcoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_erpcoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_erpcoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
